...
export default {
  ...
  data() {
    return {
      serviceUrl: 'http://localhost:8024/api/wordeditor/',
      collborativeEditingServiceUrl: "http://localhost:8024/",
      connection: null,
      ...
      buttonType: 'button',
      currentRoomName: '',
      webSocketEndPoint: 'http://localhost:8024/ws',
      stompClient: null,
    };
  },
  mounted() {
  ...
  methods: {
    ...
    onConnected() {
      if (this.stompClient.connected) {
        ...
        this.stompClient.subscribe('/topic/public/' + this.currentRoomName, this.onDataRecived);
    ...
    onDataRecived(data) {
      if (this.$refs.doceditcontainer.ej2Instances.documentEditor.collaborativeEditingHandlerModule) {
        ...
        if (content.payload.operations != null) {
          this.$refs.doceditcontainer.ej2Instances.documentEditor.collaborativeEditingHandlerModule.applyRemoteAction("action", content.payload);
        }
        else if (action == "removeUser") {
          this.$refs.doceditcontainer.ej2Instances.documentEditor.collaborativeEditingHandlerModule.applyRemoteAction("removeUser", content.payload.connectionId);
          this.updateUserInfo(content.payload.connectionId, 'removeUser');
        } else if (action == "connectionId" && this.connectionId == "") {
          this.connectionId = content.payload.connectionId;
          this.$refs.doceditcontainer.ej2Instances.documentEditor.collaborativeEditingHandlerModule.applyRemoteAction("connectionId", content.payload.connectionId);
        }
        else if (Array.isArray(content.payload)) {
          this.updateUserInfo(content.payload, 'addUser');
        }
      }
    },
    ...
    connectToRoom() {
      var userInfo = {
        currentUser: this.$refs.doceditcontainer.ej2Instances.currentUser,
        clientVersion: 0,
