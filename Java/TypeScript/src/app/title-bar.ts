...
export class TitleBar {
    ...
    public addUser(actionInfos: ActionInfo | ActionInfo[], currentConnectionId: string): void {
        if (!(actionInfos instanceof Array)) {
            actionInfos = [actionInfos]
        }
        for (let i: number = 0; i < actionInfos.length; i++) {
            let actionInfo: ActionInfo = actionInfos[i];
            if (currentConnectionId === actionInfo.connectionId || this.userMap[actionInfo.connectionId as string]) {
                continue;
            }
